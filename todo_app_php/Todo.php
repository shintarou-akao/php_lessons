...
class Todo {
  ...
  public function __construct() {
    ...
    try {
      ...
      $this->_db->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION); //第一引数；エラーの通知方法、第二引数：例外を発生
  ...
  public function getAll() {
    $stmt = $this->_db->query("select * from todos order by id desc"); //常に新しいもの一番上に
    return $stmt->fetchAll(\PDO::FETCH_OBJ); //返ってきた値をオブジェクト形式で返す
  }
  ...
  public function post() {
    ...
    switch ($_POST['mode']) {
      case 'update';
        return $this->_update(); //privateメソッドに。returnで処理が返ってくるのでbreakは必要ない。
      case 'create';
        return $this->_create();
      case 'delete';
        return $this->_delete();
  ...
  private function _update() {
    ...
    $stmt = $this->_db->query($sql); //queryでデータを引っ張ってくる
